...
10. **JSON Codex IMEDIATO apÃ³s gerador-prompt-hq:** o orquestrador escreve o JSON de pedido em **`estudos\.claude\pending\`**. SÃ£o **dois arquivos obrigatÃ³rios** escritos juntos: (a) `hq-[slug].json` para as pÃ¡ginas da HQ e (b) entrada no `portraits-batch.json` para o portrait do personagem (usando `hq-[slug]-pg1.png` como char_sheet, output em `_landing/chars/[slug]-hd.png`). Se escrever os JSONs diretamente sem usar o agente `gerador-imagens-hq`, ambos devem ser criados â€" nunca apenas o de HQ. O restante da regra: (projeto do 5Âº ano â€” Ã© esse caminho que o Codex Desktop monitora, NÃƒO `estudos-2ano\.claude\pending\`) assim que o `gerador-prompt-hq` confirmar o arquivo .md â€” sem esperar o restante do pipeline. Isso garante que o Codex processe em paralelo. Formato obrigatÃ³rio: UTF-8 sem BOM via `[System.IO.File]::WriteAllText(path, json, [System.Text.UTF8Encoding]::new($false))`; `expected_outputs` deve ser `[“chars”, “pg1”, “pg2”, “pg3”, “pg4”]` â€” SEMPRE incluir `chars` antes das pÃ¡ginas, pois o Codex gera a folha de personagens primeiro e as pÃ¡ginas da HQ usam essa referÃªncia visual. Omitir `chars` faz o Codex gerar as HQs sem referÃªncia estabelecida, e o portrait no `portraits-batch.json` deve usar `hq-[slug]-chars.png` como `char_sheet` (nÃ£o `pg1.png`); campo `raiz` apontando para `estudos-2ano`. **Backslashes no JSON:** usar string literal PowerShell single-quoted (`'...'`) com `\\` por barra â€” NUNCA interpolar variÃ¡veis de caminho com `-replace '\\','\\\\'` pois isso produz `\\\\` no arquivo (caminho invÃ¡lido). Ver ERR-006.
